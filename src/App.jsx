...
function Resumen({ transactions }) {
  ...
  const porCategoria = useMemo(() => {
    const map = {};
    filtradas.filter((t) => t.tipo === "gasto").forEach((t) => { map[t.categoria] = (map[t.categoria] || 0) + Number(t.monto); });
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="relative rounded-xl bg-white p-5 pt-7 shadow-sm">
        <ReceiptEdge />
        <h2 className="font-display mb-1 text-[17px] text-[#1C2B33]" style={{ fontWeight: 600 }}>Gastos por categoría</h2>
        <p className="mb-4 text-xs text-[#8A8E86]">Distribución del total gastado en el periodo</p>

        {porCategoria.length === 0 ? (
          <p className="text-sm text-[#8A8E86]">No hay gastos en este periodo.</p>
        ) : (
